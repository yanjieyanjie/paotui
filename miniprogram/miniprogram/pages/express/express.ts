...
import { loadAddressesFromStorage, decorateAddress } from '../../utils/address';
...
Page({
  ...
  onPickupChange(e: WechatMiniprogram.PickerChange) {
  ...
  getPickupPoint(): string {
    const d = this.data;
    if (d.isCustomPickup) return String(d.pickupCustom || '').trim();
  ...
  onSizeAdd(e: WechatMiniprogram.TouchEvent) {
    const key = e.currentTarget.dataset.key as 'small' | 'medium' | 'large';
  ...
  onSizeMinus(e: WechatMiniprogram.TouchEvent) {
    const key = e.currentTarget.dataset.key as 'small' | 'medium' | 'large';
    const qtys = this.data.sizeQtys;
