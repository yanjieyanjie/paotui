...
Page({
    ...
    async loadMessages(silent = false) {
        ...
        try {
            const list = await (0, messages_1.getChatMessages)(orderId, otherUserId);
            const me = (0, account_1.getCurrentUserId)();
            ...
            (0, messages_1.readConversation)(orderId, otherUserId).catch(() => undefined);
        }
        catch {
            this.setData({ loading: false });
            if (!silent) {
    ...
    async loadOrder() {
        ...
        try {
            const order = await (0, orders_1.getOrder)(orderId);
            this.setData({
                orderTitle: order?.title || this.data.orderTitle,
                orderStatus: order?.status || '',
                orderStatusLabel: order?.status
                    ? index_1.ORDER_STATUS_LABELS[order.status] || order.status
                    : '',
    ...
    async loadPeer() {
        ...
        try {
            const user = await (0, users_1.getUser)(otherUserId);
            if (user?.nickname) {
    ...
    async sendContent(content, type) {
        const { orderId, otherUserId } = this.data;
        if (!content || !orderId || !otherUserId) {
            return;
        }
        try {
            const created = await (0, messages_1.sendChatMessage)(orderId, otherUserId, content, type);
    ...
    appendMessage(created) {
        ...
        const item = {
            ...
            timeText: (0, format_1.formatTime)(created.createdAt),
    ...
    compressAndSendImage(src) {
        const readAndSend = (path) => {
