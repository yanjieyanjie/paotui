...
Page({
    ...
    async onSwitchAccount() {
        if (this.data.loading)
            return;
        try {
            const accounts = await (0, users_1.getUsers)();
            const items = accounts.map((a) => a.nickname + (a.phone ? `（${a.phone}）` : ''));
            wx.showActionSheet({
                itemList: items,
                success: (res) => {
                    const account = accounts[res.tapIndex];
                    if (account && account.id !== (0, account_1.getCurrentUserId)()) {
                        (0, account_1.setCurrentUserId)(account.id);
                        this.loadUser();
    ...
    async onSave() {
        ...
        try {
            await (0, users_1.updateUser)(this.data.userId, payload);
