...
Page({
  ...
  onWashAdd(e: WechatMiniprogram.TouchEvent) {
    const key = e.currentTarget.dataset.key as 'spin' | 'fast' | 'standard' | 'large';
  ...
  onWashMinus(e: WechatMiniprogram.TouchEvent) {
    const key = e.currentTarget.dataset.key as 'spin' | 'fast' | 'standard' | 'large';
    const washQtys = this.data.washQtys;
    if ((washQtys[key] || 0) > 0) washQtys[key] -= 1;
    this.setData({ washQtys });
    this.calcTotal();
  },
